        ...
        public IList<Site> ReadToListSite(int campgroundID, DateTime inputStartDate, DateTime inputEndDate)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SELECT DISTINCT site.* from site WHERE campground_id = @campgroundId AND site.site_id NOT IN (SELECT site_id FROM reservation WHERE from_date <= @inputEndDate AND to_date >= @inputStartDate) ", conn);
